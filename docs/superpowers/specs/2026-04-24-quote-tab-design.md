...
**스코프:** public 렌더 UI + DB 스키마 파일 생성 + 앱 DB 연결 코드. 어드민·API 라우트는 별도 spec. **DB/user/`.db_credentials` 생성은 이 spec 범위 밖** — 운영 선행 작업(§11) 참고.
...
| 인터랙션 | permalink (`/quote#q{id}`) 앵커 + 복사 버튼 (모바일 상시, 데스크탑 hover-reveal). 모달 없음 |
...
- `portrait_url` — **Next.js public-root 기준 경로만 허용** (예: `/members/member01.jpg`). `http(s)://` 절대 URL 은 이번 스코프에서 **금지**. NULL 이면 플레이스홀더 렌더. 원격 이미지 필요 시 이 spec 을 보완 + `next.config.ts` `images.remotePatterns` 등록 후 도입.
...
src/app/quote/page.tsx              (수정) 서버 컴포넌트로 재작성, force-dynamic
...
### `src/lib/db.ts` — lazy singleton

**중요:** 모듈 import 시점에 `.db_credentials` 파일을 읽거나 풀을 만들지 않는다. `getPool()` 첫 호출 시에만 초기화. 이렇게 해야 빌드 환경에 DB/크리덴셜이 없어도 `pnpm build` 가 깨지지 않는다 (라우트가 `force-dynamic` 이어도 다른 경로가 이 모듈을 import 하면 평가될 수 있음).
...
// Next.js hot-reload 에서 풀이 재생성되지 않도록 globalThis 싱글톤
...
소비 측(`quotes.ts` 등) 은 `db.query(...)` 대신 `getPool().query(...)` 호출. `export const db = ...` 같은 top-level eval 은 금지.
...
### `.db_credentials` 파일 포맷 (참고 — 생성은 이 spec 밖)
...
경로: `/var/www/html/_______site_BANDSUSTAIN/.db_credentials`. chmod 600. **파일이 없으면 구현을 시작하지 말고** §11 운영 선행 작업으로 돌아간다.
...
// 빌드 시점 pre-render 차단 (DB 미접속 환경에서 빌드 깨짐 방지)
...
- **Meta row (항상 렌더)** — attribution 유무와 무관하게 한 줄 확보. 구조:
  - attribution 있으면: `— {attribution} —` 를 좌측(또는 reverse 시 우측 정렬) 에 `text-xs uppercase tracking-[0.08em] text-[var(--color-text-muted)]` 로 표시
  - attribution 없으면: 이 자리 비움 (빈 span 유지)
  - 같은 row 오른쪽 끝에 `CopyPermalink` 버튼 항상 렌더
- **CopyPermalink 버튼 가시성 규칙:**
  - `< md` (모바일/터치): **항상 노출** (hover 개념 없음). `opacity-60` 기본, `active:opacity-100`
  - `≥ md` (데스크탑): 기본 숨김, row hover 또는 버튼 focus 시 노출 (`md:opacity-0 md:group-hover:opacity-100 md:focus-visible:opacity-100 md:transition-opacity`)
  - 클릭 동작: `navigator.clipboard.writeText(location.origin + '/quote#q' + id)` + 1.5초간 "copied" 토글
  - **접근성:** `aria-label="Copy permalink to this quote"`. 이 버튼만 클라이언트 컴포넌트 (`CopyPermalink.tsx`, `"use client"`).
...
- `portrait_url` 있음: `<Image src={portrait_url} alt={attribution ?? "portrait"} width={160} height={200} className="w-full aspect-[4/5] object-cover grayscale contrast-[1.05]" />`
  - **로컬 경로만 허용** (Next.js public-root 기준, `/`로 시작). 원격 URL 은 스키마 레벨에서 금지 (§3 `portrait_url` 규칙 참고).
  - 존재하지 않는 로컬 경로는 `next/image` 가 404 로 응답하며 깨진 이미지를 렌더. 운영자가 INSERT 시 경로 정확성 책임짐. 이번 스코프에서는 자동 fallback 로직 미구현.
...
- Copy 버튼은 항상 meta row 에 자리가 있음 (attribution 유무와 무관). 모바일 상시 노출, 데스크탑 hover-reveal — 세부 규칙은 §6 `QuoteRow` meta row 부분 참고.
...
- **잘못된 로컬 이미지 경로**: `next/image` 가 404 응답 + 깨진 이미지 렌더. 이번 스코프에서는 자동 placeholder fallback 미구현 — 운영자가 INSERT 시 경로 점검.
- **원격 이미지**: 스키마 레벨에서 금지 (§3). 절대 URL 이 DB 에 들어가 있을 경우 `next/image` 가 `remotePatterns` 미등록 에러로 **페이지 전체 500** 이 될 수 있음. 운영자는 로컬 경로만 INSERT.
...
```bash
# 선행 조건: §11 의 3가지(DB/user/.db_credentials) 가 이미 준비돼 있음
pnpm add mysql2
mysql -u $USER -p BANDSUSTAIN < db/schema/001_quotes.sql
mysql -u $USER -p BANDSUSTAIN < db/seed/quotes_seed.sql   # 선택
git add -A && git commit -m "Add quote tab with DB-backed content"
git push origin main
pnpm build         # DB 미접속이어도 성공해야 함 (force-dynamic + lazy pool)
...
- 브라우저 콘솔 에러/워닝 0
- `pnpm build` 성공 — **DB 접속 불가능한 환경에서도** 성공해야 함 (lazy pool + force-dynamic 가 제대로 작동하는지 확인)
- 데스크탑 `md:` 이상에서 copy 버튼이 hover 시만 노출 / 모바일 폭에서 항상 노출되는지 확인

---

## 11. 운영 선행 작업 (Preconditions — 이 spec 밖, 구현 착수 전 요구)

앱 구현은 아래 리소스가 **이미 존재한다는 전제**로 진행한다. 없으면 구현을 시작하지 않고 운영자 작업으로 핸드오프.

| 항목 | 상태 조건 | 담당 |
|------|-----------|------|
| MariaDB 에 `BANDSUSTAIN` 데이터베이스 | 존재 + utf8mb4 | 운영자 |
| DB user (CLAUDE.md 의 소유자 기준) | `BANDSUSTAIN.*` 에 SELECT/INSERT/UPDATE/DELETE 권한 | 운영자 |
| `/var/www/html/_______site_BANDSUSTAIN/.db_credentials` | 존재, chmod 600, §5 포맷 준수 | 운영자 |

**구현자 체크:**
- `/var/www/html/_______site_BANDSUSTAIN/.db_credentials` 의 존재와 read 권한 확인
- `mysql -u $DB_USER -p... -e "USE BANDSUSTAIN; SHOW TABLES;"` 성공 확인 (초기엔 테이블 없어도 됨)
- 셋 중 하나라도 실패하면 구현 중단 → 운영자에게 보고. 앱 코드가 DB/user/credentials 를 생성하지 않는다.

`quotes` 테이블 자체는 §3 의 `db/schema/001_quotes.sql` 로 구현자가 수동 실행. 스키마 파일 자체는 app repo 에 커밋됨.
...
- `BANDSUSTAIN.quotes` 테이블 (001_quotes.sql) — 구현자가 수동 실행
- `BANDSUSTAIN` 데이터베이스 + user + `.db_credentials` — **선행 작업, §11 참고**
...
2. §11 선행 작업 완료 여부 점검 (운영자 작업) → 완료 후 코드 착수
